...
def predict_and_visualize(model, img_path):
  ...
  # After prediction
  cat_ids = {} 
  for i in range(pr_mask.shape[-1]):
    cat_ids[i] = i + 1
...
def main():
    ...
    image_paths = [os.path.join(images_folder, f) for f in os.listdir(images_folder) if f.endswith('.jpg') or f.endswith('.png')]
